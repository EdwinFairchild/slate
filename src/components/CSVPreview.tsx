        ...
        <thead className="sticky top-0 bg-white dark:bg-gray-800">
          <tr>
            {headers.map((header, index) => (
              <th
                key={header}
                className={`px-3 py-2 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider ${index === 0 ? 'w-40' : 'w-auto'
        ...
        <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
          {data.map((row, i) => (
            <tr key={i}>
              {headers.map((header, index) => (
                <td
                  key={header}
                  className={`px-3 py-2 whitespace-nowrap text-sm text-gray-700 dark:text-gray-300 ${index === 0 ? 'w-32' : 'w-auto'
          ...
          <tr>
            {headers.map((header, index) => (
              <td
                key={header}
                className={`px-3 py-2 whitespace-nowrap text-sm ${index === 0 ? 'w-48' : index === 1 ? 'w-56' : 'w-auto'
                  }`} // Apply width adjustment for the first column
              >
                <div className="flex space-x-2">
                  ...
                  <input
                    ...
                    className="block w-full h-9 px-3 rounded-md border border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:text-white text-sm"
                  />
                  {/* Apply Button */}
                  <button
